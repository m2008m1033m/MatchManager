...
public class Communicator {
    ...
    public final static String BASE_URL = "http://hattasc.com/";
    ...
    public static Communicator getInstance() {
        if (mInstance == null)
            mInstance = new Communicator(MatchManagerApplication.getContext());
        return mInstance;
    }
    ...
    public void get(String url, RequestParams requestParams, AsyncHttpResponseHandler handler, String tag, Header[] headers) {
        ...
        if(requestParams != null) Log.d("Communicator", "The params are: " + requestParams.toString());
    ...
    public void postJSON(String url, RequestParams requestParams, AsyncHttpResponseHandler handler, String tag, Header[] headers) {
        try {
            ...
            mAsyncHttpClient.setTimeout(TIME_OUT);
            mAsyncHttpClient.post(MatchManagerApplication.getContext(), url, entity, "application/x-www-form-urlencoded", handler);
    ...
    private AsyncHttpResponseHandler wrapHandler(final AsyncHttpResponseHandler handler, final String tag) {
        AsyncHttpResponseHandler wrapper = new AsyncHttpResponseHandler() {
            ...
            public void onFailure(int statusCode, Header[] headers, byte[] responseString, Throwable throwable) {
                String result = responseString == null ? MatchManagerApplication.getContext().getString(R.string.unknown_error_has_occurred) : new String(responseString);

                Log.d("Communicator", "Request '" + tag + "': " + result);
                try {
                    JSONObject jsonObject = new JSONObject();
                    jsonObject.put("message", MatchManagerApplication.getContext().getString(R.string.unknown_error_has_occurred));
